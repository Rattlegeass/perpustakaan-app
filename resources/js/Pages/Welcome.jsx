...
export default function Welcome({ auth, koleksiBuku = [] }) {
  ...
  const categories = [
        { 
            id: 1, 
            title: 'Fiksi', 
            // Deskripsi yang lebih menarik, imajinatif, dan asyik
            desc: 'Bebaskan imajinasimu! Rak ini berisi kumpulan cerita seru hasil karangan para penulis. Cocok buat kamu yang ingin sejenak rehat dari dunia nyata, mencari hiburan, atau sekadar ingin senyum-senyum sendiri baca novel.', 
            isFeatured: true, 
            items: ['Jelajahi Semua Koleksi Fiksi'] 
        },
        { 
            id: 2, 
            title: 'Non-Fiksi', 
            // Deskripsi yang memotivasi untuk belajar dan upgrade diri
            desc: 'Asah wawasanmu di sini! Kumpulan buku berbasis fakta, ilmu pengetahuan, dan kisah nyata. Pilihan paling pas buat kamu yang suka belajar hal baru, mencari inspirasi, atau ingin memotivasi diri jadi lebih baik.', 
            isFeatured: false, 
            items: ['Jelajahi Semua Koleksi Non-Fiksi'] 
        }
    ];
    
    // Filter menggunakan data asli dari database
    const filteredBooks = koleksiBuku.filter(book => {
        if (popularFilter === 'Semua') return true;
        // Membandingkan dengan huruf kecil karena di database menggunakan 'fiksi' dan 'non-fiksi'
        return book.kategori.toLowerCase() === popularFilter.toLowerCase();
    }).slice(0, 10);
