...
export default function Edit({ member }) {
    ...
    const { data, setData, post, processing } = useForm({
        _method: 'put', // Required by Laravel for file uploads in PUT requests sent as POST
        name: member.name,
        email: member.email,
        no_telp: member.no_telp || '',
        no_identitas: member.no_identitas || '',
        foto: null, // Holds the newly selected file if any
    });
    ...
    const handleSubmit = (e) => {
        ...
        post(`/members/${member.id}`, {
            forceFormData: true, // Wajib diaktifkan agar file/gambar terkirim
            onError: (errors) => {
                setErrors(errors);
            },
        });
    ...
    return (
        <AuthenticatedLayout header="Edit Profil Member">
            <div className="max-w-3xl mx-auto px-4 py-6">
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-6">
                    <div>
                        <h3 className="text-xl font-bold text-slate-800">✏️ Edit Informasi Anggota</h3>
                        <p className="text-slate-500 text-sm mt-0.5">Ubah informasi profil untuk anggota {member.name}.</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <div className="space-y-2 text-center sm:text-left flex-1">
                            ...
                            <p className="text-xs text-slate-400">Pilih berkas baru (JPG, PNG, JPEG, GIF) untuk mengganti foto lama.</p>
                            <input
                                type="file"
                                id="avatar-upload"
                                accept="image/*"
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-slate-200">
                        <button
                            type="submit"
                            disabled={processing}
                            className="px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition disabled:opacity-50 text-sm shadow-md text-center"
                        >
